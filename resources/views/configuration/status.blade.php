                ...
                <div class="card-table">
                                ...
                                @foreach ($statuses as $status)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $status->slug }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $status->created_at }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.statuses.edit', $status->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                            <form action="{{ route('admin.statuses.destroy', $status->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
